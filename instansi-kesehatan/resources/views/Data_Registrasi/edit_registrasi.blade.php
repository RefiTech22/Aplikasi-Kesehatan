                ...
                <div class="card-body">
                    <div class="form-group">
                        ...
                        <select name="jenis_registrasi" class="form-control select2" style="width: 100%;">
                            <option  {{ ($registrasi[0]->jenis_registrasi == 'UGD'?'selected':'')}} value="UGD"> UGD</option>
                            <option  {{ ($registrasi[0]->jenis_registrasi == 'Rawat Inap'?'selected':'')}}  value="Rawat Inap"> Rawat Inap</option>
                            <option  {{ ($registrasi[0]->jenis_registrasi == 'Rawat Jalan'?'selected':'')}}  value="Rawat Jalan"> Rawat Jalan</option>
                        </select>
                    </div>
                    <div class="form-group">
                        <label>Status registrasi</label>
                        <select name="status_registrasi" class="form-control select2" style="width: 100%;">
                            <option  {{ ($registrasi[0]->status_registrasi == 'Aktif'?'selected':'')}} value="Aktif"> Aktif</option>
                            <option  {{ ($registrasi[0]->status_registrasi == 'Tutup Kunjungan'?'selected':'')}} value="Tutup Kunjungan"> Tutup Kunjungan</option>
                        </select>
                    </div>
                    <div class="form-group">
                        ...
                        <select name="pasien" class="form-control select2" style="width: 100%;">
                          @foreach ($registrasi as $regis)
                            @foreach ($pasien as $item)
                              <option  {{ ($regis->id_pasien == $item->id?'selected':'')}} value="{{ $item->id }}"> {{ $item->no_rekam_medis }}</option>
                        ...
                        <select name="layanan" class="form-control select2" style="width: 100%;">
                          @foreach ($registrasi as $regis)
                            @foreach ($layanan as $item)
                                <option  {{ ($regis->id_layanan == $item->id?'selected':'')}} value="{{ $item->id }}"> {{ $item->layanan }}</option>
                        ...
                        <select name="pembayaran" class="form-control select2" style="width: 100%;">
                          @foreach ($registrasi as $regis)
                            @foreach ($pembayaran as $item)
                                <option  {{ ($regis->id_pembayaran == $item->id?'selected':'')}} value="{{ $item->id }}"> {{ $item->pembayaran }}</option>
                        ...
                        <select name="petugas" class="form-control select2" style="width: 100%;">
                          @foreach ($registrasi as $regis)
                            @foreach ($petugas as $item)
                                <option  {{ ($regis->id_petugas == $item->id?'selected':'')}} value="{{ $item->id }}"> {{ $item->petugas }}</option>
