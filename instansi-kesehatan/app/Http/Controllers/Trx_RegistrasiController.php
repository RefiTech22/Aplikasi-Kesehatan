...
class Trx_RegistrasiController extends Controller
{
    public function index()
    {
        ...
        where b.id = a.id_pasien AND c.id = a.id_pembayaran AND d.id = a.id_layanan AND e.id = a.id_petugas ORDER BY id');
    ...
    {
        $QP          = Master_Pasien::orderBy('id', 'ASC')->get();
        $QL          = Master_Layanan::orderBy('id', 'ASC')->get();
        $QPembayaran = Master_Pembayaran::orderBy('id', 'ASC')->get();
        $QPetugas    = Master_Petugas::orderBy('id', 'ASC')->get();
        return view('Data_Registrasi/tambah_registrasi',[
            "title"      => "Tambah registrasi",
            "pasien"     => $QP,
            "layanan"    => $QL,
            "pembayaran" => $QPembayaran,
            "petugas"    => $QPetugas
        ]);
    }
